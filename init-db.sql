...
INSERT INTO accounts (user_id, cash_balance, available_cash) VALUES
    ('USER001', 1000000.0000, 1000000.0000),
    ('USER002', 500000.0000, 500000.0000),
    ('USER003', 2000000.0000, 2000000.0000)
ON DUPLICATE KEY UPDATE updated_at = CURRENT_TIMESTAMP(6);

-- 테스트용 주식 보유 데이터
INSERT INTO stock_holdings (user_id, symbol, quantity, available_quantity, average_price) VALUES
    ('USER001', 'AAPL', 100.0000, 100.0000, 150.0000),
    ('USER002', 'GOOGL', 50.0000, 50.0000, 2800.0000)
ON DUPLICATE KEY UPDATE updated_at = CURRENT_TIMESTAMP(6);